...
void IMUSO3Thread(void)
{
		//! Time constant
	float dt = 0.01f;		//s
	static uint32_t tPrev=0,startTime=0;	//ms
	uint32_t now;
//	uint8_t i;
...
//	static uint8_t gyroInitialized=0;
	...
	acc[2] = -imu.accb[2];
		...
		//euler[0] = -asinf(Rot_matrix[2]);	// Pitch 
		//euler[1] = atan2f(Rot_matrix[5], Rot_matrix[8]);	// Roll
		//euler[2] = atan2f(Rot_matrix[1], Rot_matrix[0]);	 //yaw
		
		
		euler[0] =  asin(Rot_matrix[5]); 	//滚动角，绕X轴
    euler[1]  = atan2(-2*q0*q2 + 2*q1*q3, -2 * q1 * q1 - 2 * q2* q2 + 1);//俯仰角，绕Y轴
		euler[2] = atan2f(Rot_matrix[1], Rot_matrix[0]);	 //yaw
